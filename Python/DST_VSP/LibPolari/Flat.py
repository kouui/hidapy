...
# 2. (estimate) a rough shift to align right panel to left panel
# [x] 3. remove padding cols and rows in right panel (left panel also)
...
def split_and_warp_(img : T_ARRAY, sw_params : Split_Warp_Params):
    ...
    for name in ("left", "right"):
        ...
        tform = tforms[name]
        img_warp : T_ARRAY = _coords_warp_spline_(panel[name], tform)
...
def find_split_and_warp_params_(img3d : T_ARRAY, 
    ...
    # 2. a rough shift to align right panel to left panel
    dyx : Dict[str, T_ARRAY] = {}
    dyx["right"] = _measure_shift2d_(panel["right"], panel["left"])
    dyx["left"] = _numpy.zeros_like(dyx["right"])
    ...
    for y0 in xy_lines["y0s"]:
        for name in ("left", "right"):
            ...
            x1 -= int( dyx[name][1] )
            x2 -= int( dyx[name][1] )
            xs, ys, coe = _poly2dfit_curve_line_(panel[name], y0, x1, x2, step_size=20)
    ...
    for x0 in xy_lines["x0s"]:
        for name in ("left", "right"):
            ...
            y2 -= int( dyx[name][0] )
            x0 -= int( dyx[name][1] )
            ys, xs, coe = _poly2dfit_curve_line_(panel[name], x0, y1, y2, step_size=50, kind=1)
    ...
    for name in ("left", "right"):
        ...
        for _, _, coe_v in lines[name]["v"]:
            ...
            for _, _, coe_h in lines[name]["h"]:
                cross[name][f"v{count}"].append( _poly2d_cross_(panel[name], coe_h, coe_v) )
...
#: this is for creating flat for shifted right panel
#  in case of creating flat, there is no need to shift the right panel
...
from ..LibIO.File import load_fits_ as _load_fits_
from ..LibIO.File import dump_fits_ as _dump_fits_
from ..LibIO.File import dump_joblib_ as _dump_joblib_
...
def stack_flats_(fnames : List[str], n : int, prefix : str = ''):
    ...
    nF = len( fnames )
    _, img3d = _load_fits_(fnames[0], verbose=False )
...
def flat3dLR_to_flat_field_all_(flat3d_LR : Dict[str,T_ARRAY], sw_params : Split_Warp_Params, original_shape : Tuple[int,int], pad_x : int = 5, pad_y : int = 10):
    ...
    for name in ("left", "right"):
        ...
        src = _numpy.append( coords_map[name]["L"][:,:2][:,::-1], coords_map[name]["R"][:,:2][:,::-1], axis=0 )
        dst = _numpy.append( coords_map[name]["L"][:,2:][:,::-1], coords_map[name]["R"][:,2:][:,::-1], axis=0 )
        nrow, ncol = flt_fld.shape
        # swap src and dst
        tform = _calculate_warp_tform_spline_(dst[:,::-1], src[:,::-1], output_region=[0,0,nrow-1,ncol-1])
        flt_fld : T_ARRAY = _coords_warp_spline_(flt_fld, tform)
        # fix padding zeros after warping back
        flt_fld[:pad_y,:] = 1.
        flt_fld[-pad_y:,:] = 1.
        flt_fld[:,:pad_x] = 1.
        flt_fld[:,-pad_x:] = 1.

        flat_field[name] = flt_fld


    xr = sw_params.xr
    flat_field_all = _numpy.ones( original_shape )
    for name in ("left", "right"):
        xr0 = xr[name]
        flat_field_all[ :, xr0[0]:xr0[1] ] = flat_field[name]

    return flat_field_all


def make_angle_flat_( fnames : List[str] , dark : T_ARRAY,
                      xy_lines : Dict[str, List[int]],
    ...
    for kA in range(nA):
        print(f"Making Flat for Waveplate Angle No.{kA:03d}")
        flat3d_LR = {}
        for name in ("left", "right") : 
            flat3d_LR[name] = _numpy.empty( (nF,)+sw_panels.panel_warp[name].shape )

        _, flatns = stack_flats_(fnames, kA)
        flatns = flatns.astype("float64")
        flatns[:,:,:] -= dark[:,:]

        for kF in range(flatns.shape[0]):
            flat_panels = split_and_warp_(flatns[kF,:,:], sw_params)
            for name in ("left","right"):
    ...
    if len(prefix) > 0:
        ...
        flat_path = os.path.join(prefix, "flats.fits")
        _dump_fits_(flat_all, "float64", flat_path)
        #print(f"Saved as : {flat_path}")
        
        sw_path = os.path.join(prefix, "sw_params.joblib")
        _dump_joblib_(sw_params, sw_path)
        #print(f"Saved as : {sw_path}")
        

    return flat_all, sw_params